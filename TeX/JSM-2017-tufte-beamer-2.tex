...
\end{equation}
\note{The matrix $T$ is the \emph{Cholesy factor}  of the precision matrix.}
\end{frame}
...
%6
\begin{frame}
\frametitle{\emph{Here's the cute part: }}
...
\begin{equation}   
\newmaththought{y\left(t_j \right)  = \sum_{k=1}^{j-1} \phi\left(t_j ,t_k\right) y\left(t_k\right) + \sigma\left(t_j\right)\epsilon\left({t_j}\right)}
\label{eq:MyModel} 
\end{equation}
\note{The regression coefficients $\phi_{jk}$ are the \emph{generalized autoregressive parameters}, and the prediction error variances $\sigma^2_{j}$ are the \emph{innovation variances}.}
\end{frame}
