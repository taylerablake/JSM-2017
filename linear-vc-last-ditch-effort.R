...
myGrid <- myGrid %>% transform(.,l_s=l/max(myGrid$l),
                               m_s=m/max(myGrid$m))
phi <- (myGrid$s)/max(myGrid$s) - 0.5
T_mod <- diag(M)
T_mod[lower.tri(T_mod)] <- -phi

D <- diag(rep(0.3,M))
Omega <- t(T_mod) %*% (solve(D)^2) %*% T_mod
Sigma <- solve(Omega)
...
# load(file.path("/Users/taylerblake/GitRepos/JSM-2017","data",
#                "linear-vc-N-30-M-20-pordl-2-pordm-2.Rdata"))
...
png(file.path("TeX",
              "img",
              paste0("linear-vc-model-true-cholesky",".png")))
...
png(file.path("TeX",
              "img",
              paste0("linear-vc-model-estimated-cholesky",".png")))
persp(z=diag(M)-fit_list[[780]]$T_mod,
      phi=15,
      theta=10,
      xlab="s",
      ylab="t",
...
png(file.path("TeX",
              "img",
              paste0("linear-vc-model-true-covariance",".png")))
...
png(file.path("TeX",
              "img",
              paste0("linear-vc-model-estimated-covariance",".png")))
persp(x=seq(0,1,length.out=M),
      y=seq(0,1,length.out=M),
      z=solve(omega_list[[780]]),
      phi=15,
      theta=15,
      xlab="s",
      ylab="t",
...
fileName <- paste0("linear-vc","-N-",N,"-M-",M, "-pordl-", dl, "-pordm-", dm)
