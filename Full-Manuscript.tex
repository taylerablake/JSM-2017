...
%\begin{equation}
%-2L + \hat{\lambda}_1 J_1\left(\phi^*\right) + \lambda_2 J_2\left(\phi\right) \label{stage2obj}
%\end{equation}
%\end{itemize}
\noindent
...
%For ease of exposition, we present the optimization problem in matrix notation. Let $K$ be the $p \times p$ matrix with $ij^{th}$ entry given by $R^1\left( \left(l,m\right)_i, \left(l,m\right)_j \right)$, $B$ defined as in the previous theorem, and let $D$ be the $\left( \sum_{i=1}^N n_i - N\right) \times \left( \sum_{i=1}^N n_i - N\right)$ diagonal matrix with diagonal entries $\sigma_{t_{12}}^2, \sigma_{t_{13}}^2, \dots,\sigma_{t_{1n_1}}^2, \dots, \sigma_{t_{{N},{n_N}}}^2$. Lastly, let $Y = \left(y_{t_{11}}, y_{t_{12}}, \dots, y_{t_{1,n_1}},y_{t_{21}},\dots,y_{\textsc{\relsize{-4}\textsc{{$N,n_N$}}}}\right)^T$ denote the $\sum_{i=1}^N n_i \times 1$ vector of pooled observations, and let% $Y_{\textsc{ {\textsl{(-1)}}}}$ and %$Y_{\textsc{ {\textsl{(-n)}}}}$ denote the $\left( \sum_{i=1}^N n_i - N\right)\times 1$ vectors of pooled observations with the first within-subject and last within-subject observations omitted, respectively, holding the responses and corresponding regression covariates in \eqref{MyModel}.   Rewrite \eqref{stage1obj} as

%\begin{eqnarray}
%\nonumber
%-2L + \lambda_1J_1\left(\phi^*\right) &=& \left(Y_{\textsc{ {\textsl{(-1)}}}}  - Z \Phi \right)^T D^{-1} \left(Y_{\textsc{ {\textsl{(-1)}}}} - Z \Phi \right) + \lambda_1 c^T K c \\
%&=& \left(Y_{\textsc{ {\textsl{(-1)}}}} - Z\left(Bd + Kc\right) \right)^T D^{-1} \left(Y_{\textsc{ {(-1)}}}  - Z\left(Bd + K c \right) \right) + \lambda_1 c^T K c 
%\end{eqnarray}
%\noindent 
%for appropriately specified $\left(\sum \limits_{i=1}^N n_i - N \right) \times p$ design matrix $Z=Z\left(Y_{\textsc{ {\textsl{(-n)}}}} \right)$ where $\Phi$ denotes the vector with elements given by $\phi\left(s,t\right)$ evaluated at the unique pairs of observed time points. Taking derivatives, we have that
% 
%\begin{eqnarray}
%\frac{\partial}{\partial c}\left[ -2L + \lambda_1 J_1\left(\phi^*\right)\right] &=& - \left(Z K \right)^T D^{-1}\left[Y_{\textsc{ {\textsl{(-1)}}}} - Z \left(B d + K c \right) \right] + \lambda_1 K c= 0 \label{eq:normaleq1}\\
%\frac{\partial}{\partial d}\left[ -2L + \lambda_1 J_1\left(\phi^*\right) \right]&=& - \left(Z B \right)^T D^{-1}\left[Y_{\textsc{ {\textsl{(-1)}}}} - Z \left(B d + K c \right) \right] = 0 \label{eq:normaleq2}
%\end{eqnarray}
%\noindent
%We may write $B$ according to its Q-R decomposition: 
%\[
%B  = \left[ \matQ_1 \; \matQ_2\right]\left[ \begin{array}{c} \matR \\ 0 \end{array}\right] = \matQ_1 \matR  
%\]
%
%\noindent
...
%\begin{eqnarray*}
%\tilde{c} &=&  \matQ_2\left[ \matQ_2^T\left[K + \lambda_1\left( Z^T D^{-1} Z \right)^{-1}\right] \matQ_2 \right]^{-1} \matQ_2^T\left(Z^T D^{-1} Z \right)^{-1}Z^T D^{-1} Y_{\textsc{ {\textsl{(-1)}}}}\\
%\tilde{d} &=& \matR^{-1}\matQ_1^T\left[ \left( Z^T D^{-1} Z \right)^{-1}Z^T D^{-1} Y_{\textsc{ {\textsl{(-1)}}}} -  \left[K + \lambda_1\left( Z^T D^{-1} Z \right)^{-1}\right] \hat{c} \right]
%\end{eqnarray*}
%\noindent
...
%
%We fix $\lambda_1 = \hat{\lambda}_1$ at the value of $\lambda_1$ for which $\left( \tilde{c}\left(\lambda_1\right),\tilde{d}\left(\lambda_1\right)\right) $ minimize $-2L + \lambda_1 J_1\left({\phi}^*\right)$. 
...
%See cite{rothman2010new} for proof that if $T$ is k-banded, then $\Sgima^{-1}$ is k-banded, and explain why this is attractive for regularization of elements on the inverse covariance matrix. Also explain using conditional covariances why it is attractive to have conditional dependence as defined through the magnitude of $\phi_1^*$ diminishing as $l$ increases.
...
\underline{\bf Proof:} We may easily derive the covariance between two measurements $Y_{j}$ and $Y_{k}$ conditional on the remaining measurements $\lbrace Y_{l} \vert l\ni j,k \rbrace$ %using properties of the Normal distribution. Partition $Y = \left(Y_{1},Y_{2},\dots,Y_{,n}\right)^T$ as follows: letting $Y_{{ {\textsl{-(jk)}}}}$ denote the full random vector with the $j^{th}$ and $k^{th}$ observation omitted, we let

%\[
%Y = \left[ \begin{array}{c}  Y_{j}\\ Y_{k}\\ \hdashline Y_{{ {\textsl{-(jk)}}}} \end{array}\right]
%\]
\noindent
which leads to the corresponding partition of $\Sigma = Cov\left(Y\right)$:

%\begin{equation}
%\Sigma = \left[\begin{array}{c:c}  \Sigma_{jk} &  \Sigma_{{ {(jk),\textsl{-(jk)}}}} \\ \cdashline{1-2}  \hdashline \Sigma_{{ {\textsl{-(jk)}},(jk)}}  & \Sigma_{{ {\textsl{-(jk)}}}}   \end{array}\right] \equiv \left[\begin{array}{c:c} \Sigma_{11} & \Sigma_{12}  \\ \cdashline{1-2}  \hdashline \Sigma_{21} & \Sigma_{22}  \end{array}\right] \label{partition}
%\end{equation}
...
%\begin{equation}
%\Sigma_{11} - \Sigma_{12}^T\Sigma_{22}^T\Sigma_{12} = \Sigma_{jk} - \Sigma_{{ {(jk),\textsl{-(jk)}}}}\Sigma_{22}^{-1}\Sigma_{{ {\textsl{-(jk)}},(jk)}} \label{condcov}
%\end{equation}
%\noindent
...
%\begin{equation}
%J_2 = \sum_{  l_i \in \mathcal{L}: l_i > L} \vert \mu^* + \phi^*_1\left(l_i\right) \vert  \label{bandedpenalty}
%\end{equation}
%\noindent
...
%&=& \vert \vert \bfbeta  \vert \vert^2 + \vert \vert \eta  \vert \vert^2
%\end{eqnarray}
%
%
%
...
%\begin{eqnarray}
%\nonumber
%-2L + \hat{\lambda}_1 J_1\left(\phi\right) + \lambda_2 J_2\left(\phi\right) = \sum_{i=1}^N \sum_{j=2}^{n_i}\sigma\left({t_j}\right)^{-2} \left(y\left({t_{ij}}\right) - \sum_{k=1}^{j-1}\phi\left({t_{ij},t_{ik}}\right)y\left({t_{ik}}\right) \right)^2\mbox{\;\;\;\;\;\;\;\;\;\;\;\;\;}\\  \mbox{\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;} + \hat{\lambda}_1 J_1\left(\phi^*\right)  + \lambda_2 \sum_{i=1}^{\vert \vert \mathcal{L} \vert \vert - 1} \omega_i \frac{\left[\vert\phi_1^*\left( l_{i+1} \right)\vert - \vert\phi^*_1\left( l_{i} \right)\vert\right]_+}{l_{i+1} - l_{i}} \label{penlik1}
%\end{eqnarray}
%
%\noindent
...
%\begin{eqnarray}
%\nonumber
%-2L + \hat{\lambda}_1 J_1\left(\phi^*\right) + \lambda_2 J_2\left(\phi^*\right) =  \sum_{i=1}^{N} \sum_{j=1}^{n_i} \left(  y_{ij} - \sum_{k=1}^{j-1} \phi_{ik}y_{ik} \right)^2  
...
%
%
%\subsection{Constrained optimization: finding $\hat{c}$, $\hat{d}$}
%
% Let $Delta$ be the $\left(\vert \vert \mathcal{L} \vert \vert - 1 \right) \times \vert \vert \mathcal{L} \vert \vert $ differencing matrix, and let $K_{l}$ be the $p \times p$ matrix with $ij^{th}$ entry defined by the decomposition of $K=$ according to \eqref{TPRK}. Let $\matU$ be the matrix defined so that we may write the $\vert \vert \mathcal{L} \vert \vert -1$ constraints given by \eqref{xi_constraints} in the following matrix form: 
%\[
%\left[ \phi_1^*\left( l_{2}\right)-\phi^*_1\left( l_{1}\right),\phi_1^*\left( l_{3}\right)-\phi^*_1\left( l_{2}\right),\dots,\phi_1^*\left( l_{\textsc{ {\textsl{$\vert\vert \mathcal{L}\vert\vert$}}}}\right)-\phi^*_1\left( l_{\textsc{ {\textsl{$\vert\vert \mathcal{L}\vert\vert$ -1}}}}\right) \right]^T  =Delta \matU \begin{bmatrix}B& K_l \end{bmatrix} \begin{bmatrix} d\\ c \end{bmatrix} \le \bfxi
%\]
%\noindent
%where $\xi$ is the vector of $\vert \vert \mathcal{L} \vert \vert -1$ slack variables. Let $\bfomega = \left[ \frac{\omega_2}{l_2-l_1}, \frac{\omega_1}{l_3-l_2}, \dots,\frac{\omega_{\textsc{ {\textsl{$\vert\vert \mathcal{L}\vert\vert$ -1}}}}}{l_{\textsc{ {\textsl{$\vert\vert \mathcal{L}\vert\vert$}}}}-l_{\textsc{ {\textsl{$\vert\vert \mathcal{L}\vert\vert$ -1}}}}} \right]^T$ and $\bfa = \begin{bmatrix} d & c \end{bmatrix}^T$. We introduce lagrangian terms $\bfalpha$, $\bfgamma$ for the constraints given in \eqref{xi_constraints} so that we may write \eqref{stage2obj} 
%
%\begin{equation}
%L_\mathcal{P} = -\bfb^T\bfa + \frac{1}{2} \bfa^T  H \bfa + \bfomega^T \bfxi - \bfalpha^T  \left( \matC\bfa + \bfxi  \right) - \bfgamma^T \bfxi
%\end{equation}
%where $\matC = Delta \matU \begin{bmatrix}B& K_l \end{bmatrix}$ and $\bfa = \begin{bmatrix} d^T & c^T \end{bmatrix}^T$. Taking partial derivatives and setting them to zero, we have
%\begin{eqnarray}
%\frac{\partial}{\partial \bfa} L_{\mathcal{P}} &=& -\bfb +  H\bfa - \matC^T\bfalpha \Leftrightarrow \bfa =  H^{-1}\left(\matC^T\bfalpha + \bfb\right) \label{normeq1} \\
%\frac{\partial}{\partial \bfxi} L_{\mathcal{P}} &=& \bfomega \bfalpha - \bfgamma = 0 \Leftrightarrow \bfgamma = \bfomega - \bfalpha \label{normeq2}
%\end{eqnarray}
%
%Substituting \eqref{normeq1} and \eqref{normeq2} into $L_\mathcal{P}$, we can show that the dual problem as the form 
%\begin{equation}
%\mathop{\min}_{\bfalpha} -\frac{1}{2} \bfalpha^T\matC  H^{-1}\matC^T\bfalpha + \bfb^T  H^{-1}\matC^T\bfalpha\;\;\;\mbox{subject to }\bfo \le \bfalpha \le \bfomega 
%\end{equation}

%\[
%\bfb = -2Y_{\textsc{ {\textsl{(-1)}}}}^T\begin{bmatrix} Z B & \bfo\\ \bfo & Z K \end{bmatrix}\;\;\;\;\; H = \begin{bmatrix} B^T Z^T D^{-1}Z B  & \bfo\\ \bfo & \hat{\lambda}_1K Z^T D^{-1}Z K \end{bmatrix}
%\]
