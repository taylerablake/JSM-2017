...
Sigma <-  diag(rep(0.3),M)
Omega <- solve(Sigma)
D <- diag(diag(Sigma))
phi <- rep(0,sum(lower.tri(D)))
T_mod <- diag(M)
...
lambdas <- expand.grid(lam_l=exp(seq(-3,10,length.out=30)),
                       lam_m=exp(seq(-3,10,length.out=30)))
...
fileName <- paste0("identity-cov","-N-",N,"-M-",M, "-pordl-", dl, "-pordm-", dm)
