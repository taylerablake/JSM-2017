...
We refer to the regression coefficients $\lbrace \phi_{jk} \rbrace$ as the \emph{generalized autoregressive parameters}, or GARPs and the $\lbrace \sigma_{j} \rbrace$ as the \emph{innovation variances} or IVs. Given an i.i.d. sample $Y_1,Y_2,\dots, Y_N$, estimation of model parameters can be carried out as in the usual regression setting. 

To relax the assumption that individuals must share a common set of equally-spaced observation times, we take $Y$ and $e$ to be the discrete versions of the stochastic processes: $Y\left(t\right)$ and $e\left(t\right)$.  We assume $Y\left(t\right)$ has covariance function $G\left(s,t\right)$, and
\[
e\left(s\right) \sim \mathcal{WN}\left(0,1\right)
\] 
is a zero mean Gaussian white noise process with unit variance. It is reasonable to assume that if $Y$ is reasonably well-behaved, then $G\left(s,t\right)$ satisfies some smoothness conditions, where smoothness is defined in terms of square integrability of certain derivatives. We view the entries of $\Sigma_i$ as values of $G$ evaluated at the distinct pairs of observed time points on individual $i$. 
...
\subsection{Regularization of $T$}
...
\section{Outline Smoothing Spline Approach}

\subsection{Introduce specialized penalties for the smoothing spline decomposition}
